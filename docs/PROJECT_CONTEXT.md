...
**主代码：** `scripts/game.js`（约 15000 行单文件）

**样式：** `styles/game.css`

**BGM：** `audio/music-generator.js`（Web Audio API 程序化合成器）

**测试：** `tests/smoke.js`（37/37 通过）
...
- **Boss 21 种**：巨兽坦克、幻影坦克、要塞坦克、虚空坦克、风暴坦克、观星者坦克、废铁巨像、雷霆执政官、轨道炮台、圣龛守卫、星象仪、缝合巨兽、双子坦克、迅影、圣龛织者、灰域剑圣、陷阱师、镜像体、沙暴、重力锚、多头蛇
...
1. 改动后运行 `node tests/smoke.js`，当前基线为 37/37 通过
2. git commit + push 到 GitHub
3. 用户直接双击 `index.html` 测试（或 `http://localhost:8080`）

## 已知问题

- 部分 Boss 的弹幕逻辑仍需继续强化差异化和可读性。
- 敌人 AI 的阵营与类型差异仍不足，是后续玩法优化重点。
- 系统数量较多，新玩家认知负担仍需通过 UI 与反馈逐步降低。
...
| `scripts/game.js` | 主循环与主要游戏逻辑（约 15000 行） |
| `styles/game.css` | 全部样式 |
| `audio/music-generator.js` | 程序化 BGM |
| `tests/smoke.js` | 冒烟测试 |
| `docs/OPTIMIZATION_ROADMAP.md` | 优化路线表 |
| `docs/WORLD_SYSTEMS.md` | 世界观设定 |
| `docs/world_lore.md` | 世界观文案 |
